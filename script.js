...
function generateQuizOptions() {
    ...
    setTimeout(() => {
        ...
        // 選択肢ボタンを生成
        setTimeout(() => {
            // 正解位置をランダム化 (0, 1, 2のいずれか)
            const correctPosition = Math.floor(Math.random() * 3);
            const shuffledOptions = [...currentQuiz.options];
            
            // 正解を指定位置に配置
            if (currentQuiz.correct !== correctPosition) {
                const temp = shuffledOptions[correctPosition];
                shuffledOptions[correctPosition] = shuffledOptions[currentQuiz.correct];
                shuffledOptions[currentQuiz.correct] = temp;
            }
